...
class TestBuildPrompt:
    ...
    def test_length_matches_candidates(self) -> None:
        records = [_make_record(), _make_record(value="0.003", unit="none", data_type="p-value")]
        prompt = build_prompt(records)
        parsed = json.loads(prompt)
        assert len(parsed) == 2
...
class TestParseLlmResponse:
    ...
    def test_valid_json_array(self) -> None:
        data = [{"id": 0, "value": "32.4", "unit": "mg/dL",
                 "data_type": "measurement", "relationship": "LDL reduction",
                 "confidence": 0.95}]
    ...
    def test_markdown_fenced_json(self) -> None:
        data = [{"id": 0, "value": "0.001", "unit": "none",
                 "data_type": "p-value", "relationship": "significance",
                 "confidence": 0.99}]
        fenced = f"```json\n{json.dumps(data)}\n```"
    ...
    def test_missing_id_uses_fallback(self) -> None:
        ...
        data = [{"id": 0, "value": "32.4", "unit": "mg/dL",
                 "data_type": "measurement", "relationship": "",
                 "confidence": 0.9}]
    ...
    def test_json_embedded_in_prose(self) -> None:
        ...
        data = [{"id": 0, "value": "42", "unit": "mg",
                 "data_type": "measurement", "relationship": "dose",
                 "confidence": 0.88}]
...
class TestParseCandidates:
    ...
    def test_batch_splitting(self) -> None:
        ...
        def side_effect(**kwargs: Any) -> MagicMock:
            ...
            resp_data = [
                {"id": item["id"], "value": item["value"], "unit": "none",
                 "data_type": "measurement", "relationship": "", "confidence": 0.8}
    ...
    def test_partial_batch_failure_other_batches_succeed(self) -> None:
        ...
        call_results = [
            RuntimeError("fail"),  # first batch fails
            RuntimeError("fail"),  # retry 1
            RuntimeError("fail"),  # retry 2 (max_retries=2 => 2 attempts total)
            _make_openai_response(json.dumps([{  # second batch succeeds
                "id": 0, "value": "2", "unit": "none",
                "data_type": "measurement", "relationship": "dose", "confidence": 0.9
            }])),
        ]
        mock_client = MagicMock()
